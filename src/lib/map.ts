 ...
 */
 ...
 export default class MapService extends BaseMapService<Map> {
   ...
   public async init(): Promise<void> {
     ...
     this.map.project = (lonlat:[number,number]) => this.map.getProjection().project(new Point(...lonlat));
     this.map.unproject = (lonlat:[number,number]) => this.map.getProjection().unproject(new Point(...lonlat));
   ...
   protected handleCameraChanged = (e?: any) => {
     ...
     (this.viewport as IViewport).syncWithMapCamera({
       ...
       // aspect:
       // cameraPosition,
       // offsetOrigin: [cameraPosition[0], cameraPosition[1]],
     });
 
     // this.viewport.setScale(this.map._glScale);
     // this.viewport.setFocalDistance(this.map.cameraCenterDistance)
     // this.viewport.setModelMatrix(this.map.cameraWorldMatrix)
     // this.viewport.setViewMatrix(this.map.viewMatrix);
     // this.viewport.setViewMatrixUncentered(this.map.viewMatrix);
     // this.viewport.setProjectionMatrix(this.map.projMatrix);
     // this.viewport.setViewProjectionMatrix(this.map.projViewMatrix);
     // this.viewport.setViewProjectionMatrixUncentered(this.map.projViewMatrix);
